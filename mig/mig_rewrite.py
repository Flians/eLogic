...
def rewrite_dp(graph: nx.DiGraph, K: int):
    ...
    for n in nx.topological_sort(graph):
        if n in inputs:
            continue
        # update K-cuts
        if pt[n][0] and last_n is not None:
        ...
        for cut in cuts:
            ...
            if len(cone - cut) < 3 or len({n for n in cone - cut if graph.nodes[n]["type"] == 'M'}) < 3:
                continue  # {y} = M(a,b,c) covered by {y,a,b,c}
            else:
                subgraph = graph.subgraph(cone)
                expr = graph_to_egg_expr(subgraph, cut)
                expr_opt, inital_cost, final_cost = mig_egg.simplify(expr[0])
